...
2. **提取主体**：从剧本中提取出主要角色/主体，为每个主体写好主体描述，保存到 `subjects/` 目录。**生成主体图片时，提示词中必须说明图片比例为16:9**
3. **提取场景**：从剧本中提取出各个场景，为每个场景写好场景描述。**生成场景图片时，提示词中必须说明图片比例为16:9**
...
6. **生成首帧描述**：根据每个分镜的内容，为每个分镜写好第一帧画面描述（静态画面，避免视频化表述），保存到 `first-frames/` 目录。**默认将分镜中涉及的所有主体和场景作为参考图写入首帧描述**，即使图片尚未生成，也要使用默认URL格式填入（如：`subjects/角色名.png`, `scenes/场景名.png`）。**生成首帧图片时，提示词中必须说明图片比例为16:9**。**重要**：当提供了主体和场景参考图时，不要重复描述主体形象和场景细节，应重点描述位置关系（如"将图1中的闹钟放置到图2的餐桌的花瓶旁边位置"）或人物表情和动作
...
- **首帧提示**: 静态画面描述（避免视频化表述），包含画面内容、美学控制（光源、光线环境、景别、视角、镜头、构图、氛围等）和风格化（画面风格语言，例如"赛博朋克"、"勾线插画"、"废土风格"等）。**必须**在提示词中明确说明图片比例为 **16:9**
```
...
- ✅ **必须**在首帧提示词中明确说明图片比例为 **16:9**（如"图片比例16:9"或"aspect ratio 16:9"）
- ✅ **当提供了主体和场景参考图时**：
  - ❌ **禁止**重复描述主体形象和场景细节（参考图已包含这些信息）
  - ✅ **重点描述**位置关系：简单说明主体在场景中的位置（如"将图1中的闹钟放置到图2的餐桌的花瓶旁边位置"）
  - ✅ **有人物时**：重点描述人物表情、动作、姿态等动态信息，而非重复描述人物外观
...
### 图片比例要求
- **必须**：在生成主体图片时，提示词中**必须**明确说明图片比例为 **16:9**
- 示例：在主体描述中添加"图片比例16:9"或"aspect ratio 16:9"
...
### 图片比例要求（必需）
**所有图片生成任务（包括主体、场景、分镜首帧）都必须在提示词中明确说明图片比例为 16:9**

#### 适用范围
- ✅ **主体图片**：生成主体图片时，提示词中必须包含"图片比例16:9"或"aspect ratio 16:9"
- ✅ **场景图片**：生成场景图片时，提示词中必须包含"图片比例16:9"或"aspect ratio 16:9"
- ✅ **首帧图片**：生成分镜首帧图片时，提示词中必须包含"图片比例16:9"或"aspect ratio 16:9"

#### 示例
- 主体描述示例：`角色外观描述...3D卡通风格，圆润体型，大眼睛...图片比例16:9`
- 场景描述示例：`城市街道场景，现代建筑，车流如织...图片比例16:9`
- 首帧提示示例：`静态画面描述...暖色调，中景，中心构图...图片比例16:9`
